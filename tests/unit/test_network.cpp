...
INSTANTIATE_TEST_SUITE_P(
  ...
  testing::Values(
    ...
    std::make_tuple("10.23.42.10", net::net_e::LAN),
    std::make_tuple("172.31.255.254", net::net_e::LAN),
    std::make_tuple("192.168.42.250", net::net_e::LAN),
...
TEST_F(BindAddressTest, ConfiguredIPv4Address) {
  ...
  config::sunshine.bind_address = "192.168.42.100";

  const auto bind_addr = net::get_bind_address(net::af_e::IPV4);
  ASSERT_EQ(bind_addr, "192.168.42.100");
}
